    ...
    std::lock_guard<std::mutex> & /* queue_lock */)
{
    ...
    for (const String & produced_part_name : entry->getProducedPartNames())
    {
        /// Update mutations status.
        /// To complete each mutation with a block number bigger than part_info.getDataVersion()
        /// we would need to mutate the part `produced_part_name`.
    ...
    std::unique_lock<std::mutex> & /* queue_lock */)
{
    ...
    for (const String & source_part_name : entry->source_parts)
    {
        ...
        /// Each `source_part_name` should become Obsolete as a result of executing the entry.
    ...
    for (const String & produced_part_name : entry->getProducedPartNames())
    {
        if (is_successful)
        {
            current_parts.add(produced_part_name);
        }
        else
        {
            /// Update mutations state.
            /// Because execution of the entry is unsuccessful, `produced_part_name` will never appear
...

        {
            ...
            for (const ReplicatedMergeTreeMutationEntryPtr & entry : new_mutations)
            {
                ...
                for (const auto & pair : entry->block_numbers)
                {
                    ...
                    if (block_num > 0)
                    {
                        /// Initialize `mutation.parts_to_do`. First, we would need to mutate parts in
                        /// the `current_parts`.
                        MergeTreePartInfo covering_part_info(
                            partition_id, 0, block_num - 1, MergeTreePartInfo::MAX_LEVEL, block_num - 1);
                        mutation.parts_to_do += current_parts.getPartsCoveredBy(covering_part_info).size();
                    }
                }
                ...
                {
                    for (const String & produced_part_name : queue_entry->getProducedPartNames())
                    {
                        auto part_info = MergeTreePartInfo::fromPartName(produced_part_name, format_version);
